        ...
        try {
            const endpoint =
                user.role === 'lecturer'
                    ...
                    : user?.role === 'student' ? Apis.auth.fetch_student_metrics :
    ...
    const fetchStudentGrades = async () => {
        try {
            ...
            if (res.status === 200) {
                ...
                setGrades({
                    data: res.data,
                    weightedAverage: res.weightedAverage
                });
            }
        } catch (error) {
            handleApiError(error);
    ...
    return (
        <div className="min-h-screen z-40 bg-gray-50 flex">
            ...
            <div className="flex-1 flex flex-col">
                <div className="fixed right-0 lg:w-[81.1%] w-[100%]">
                    <Header />
                </div>
                <main className="flex-1 lg:ml-[15rem] mt-[5rem] lg:mt-[5rem] overflow-y-auto">
                    {children}
                </main>
            </div>
        </div>
