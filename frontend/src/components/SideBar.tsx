...
import {
  BarChart3, Book, BookOpen, Brain, Home, LogOut,
  LucideBookMarked, LucideGraduationCap, Settings, Workflow,
...
const SideBar = () => {
  ...
  const buttons = [
    ...
    { label: 'AI Assistant', icon: Brain, path: '/student/ai_assistant', roles: ['student'] },
  ...
  return (
    <div
      className={`transition-all duration-300 hidden lg:flex flex-col border-r border-gray-200 bg-white h-full fixed top-0 z-50 ${
    ...
    >
      <div className="p-5  border-gray-200 flex items-center justify-between">
        ...
        <button onClick={toggleSidebar} className="text-blue-500 cursor-pointer">
          {isExpanded ? <ChevronsLeft size={24} /> : <ChevronsRight size={24} className='pr-2' />}
        </button>
      </div>
      ...
      <div className="p-4 border-t border-gray-200">
        <button
          onClick={logout}
          className="w-full flex cursor-pointer items-center space-x-3 px-4 py-3 rounded-lg text-red-600 hover:bg-red-50 transition-all duration-200"
        >
